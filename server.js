...
io.on('connection', (socket) => {
    ...
    socket.on('joinGame', async (userId) => { // Mark as async for potential DB check on join
        ...
        // Optional: Check if profile exists, though our RPC handles creation.
        // This is more for logging or if you wanted to load profile data on join.
        ...
        // if (profileError) {
        //     console.error(`Error checking profile for ${trimmedUserId} on join:`, profileError.message);
        //     // Decide if this is a critical failure or if you proceed
        // } else if (existingProfile) {
        //     console.log(`User "${trimmedUserId}" already has a profile in the database.`);
        // } else {
        //     console.log(`User "${trimmedUserId}" does not have a profile yet. It will be created on their first turn.`);
        // }
    ...
    socket.on('takeTurn', async () => {
        ...
        if (actingPlayerNumber === currentPlayerTurn) {
            ...
            // --- Call the Supabase RPC function to increment turn count ---
            ...
            if (rpcError) {
                ...
                // Optionally, inform the client or handle this error (e.g., retry, log extensively)
                // For now, the game proceeds, but the turn might not have been counted.
            ...
            // Update message to reflect DB attempt
            // The specific success/fail message is already sent above based on rpcError
            // So we just need the general turn switch message now.
    ...
    socket.on('disconnect', () => {
        ...
        if (disconnectedPlayerNumber) {
            ...
            if (remainingPlayerSocketIds.length === 1) {
                ...
                const remainerData = players[soleRemainingSocketId]; // Get the full player object
                
                players[soleRemainingSocketId] = { playerNumber: 1, userId: remainerData.userId }; // Update playerNumber
                playerSockets[1] = playerSockets[remainerData.playerNumber]; // Move socket to P1 key

                if (remainerData.playerNumber === 2) { // If the one remaining was P2
